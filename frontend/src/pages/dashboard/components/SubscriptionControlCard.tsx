    ...
    <Panel className="space-y-4">
      ...
      <form className="space-y-4" onSubmit={onCheckout}>
        <label className="grid gap-2 text-sm muted-copy">
          Plan
          <select {...checkoutForm.register("planId")}>
            {plans.map((plan) => <option key={plan._id} value={plan._id}>{plan.name} · {currency(plan.amountInr)}</option>)}
          </select>
        </label>
        <Button type="submit" disabled={plansLoading}>Continue to payment</Button>
        {plansError ? <p className="text-sm text-amber-700">Using fallback plans because live plan fetch failed.</p> : null}
      </form>
