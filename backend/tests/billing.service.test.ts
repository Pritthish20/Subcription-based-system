...
const mocks = vi.hoisted(() => ({
  ...
  getEnv: vi.fn(() => ({ APP_URL: "http://localhost:5173" }))
}));
...
describe("billing service flows", () => {
  ...
  it("creates a Razorpay donation checkout order when the provider is configured", async () => {
    ...
    mocks.getEnv.mockReturnValue({ APP_URL: "http://localhost:5173", RAZORPAY_KEY_ID: "rzp_test_123", RAZORPAY_KEY_SECRET: "secret" });
  ...
  it("verifies a Razorpay donation payment and records the donation ledger", async () => {
    mocks.getEnv.mockReturnValue({ APP_URL: "http://localhost:5173", RAZORPAY_KEY_SECRET: "secret" });
  ...
  it("skips duplicate processed Razorpay webhook events", async () => {
    ...
    mocks.getEnv.mockReturnValue({ APP_URL: "http://localhost:5173", RAZORPAY_WEBHOOK_SECRET: "whsec_test" });
